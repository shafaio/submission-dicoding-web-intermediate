...
export async function unsubscribe() {
  ...
  try {
    ...
    // 1. Coba unsubscribe dari browser
    const unsubscribed = await pushSubscription.unsubscribe();
    console.log("Unsubscribe success?", unsubscribed);
    if (!unsubscribed) {
      alert(failureMessage);
      return;
    }

    // 2. Jika berhasil, kirim DELETE ke server
    const { endpoint } = pushSubscription.toJSON();
    const response = await unsubscribePushNotification({ endpoint });
    if (!response.ok) {
      console.error("unsubscribe: server failed:", response);
      alert("Client berhasil unsubscribe, tapi server gagal sinkron.");
      return;
    }
